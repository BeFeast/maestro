...
func TestApprovalGroupKey_GroupsByTarget(t *testing.T) {
	cases := []struct {
		...
		{
			// Greptile Comment 3 on the previous attempt: when both target
			// and id are absent, returning a shared literal "ungrouped"
			// key folded unrelated approvals onto one card with a bogus
			// «regenerated N times» badge. Empty string here lets the
			// fleet aggregator skip them via the `if GroupKey == ""` guard.
			name: "no_target_no_id_returns_empty_for_caller_to_skip",
			a:    &state.Approval{},
			want: "",
		},
	}
