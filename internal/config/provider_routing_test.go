...
func TestExplicitFallbackChainIgnoresUnusableFleetProviderLanes(t *testing.T) {
	cfg, err := Parse([]byte(`
repo: owner/repo
model:
...
`))
	...
	if cfg.Model.Default != "claude" {
		t.Fatalf("default = %q, want built-in claude", cfg.Model.Default)
	}
	if got := cfg.Model.ResolvedRoute(); got.SelectionReason != ModelRouteExplicitBackendChain || !reflect.DeepEqual(got.Backends, []string{"claude", "codex"}) {
		t.Fatalf("route = %+v, want legacy explicit chain", got)
	}
}
