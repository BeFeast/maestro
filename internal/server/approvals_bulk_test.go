...
func TestBulkApproval_MixedTerminalAndPending_SkipsTerminalNotErrors(t *testing.T) {
	...
	if !resp.OK {
		t.Fatalf("ok = false despite a successful apply with only skipped sidecar; resp = %+v", resp)
	}
}

// TestBulkApproval_UnknownID_RollsBackEntireBatch protects the
// all-or-nothing contract reported in code review (#533 Comment 1): a
// batch containing even one unknown id must NOT persist mutations on
// the valid ids. The pre-validation pass rejects the whole request
// with 400, valid ids come back as "skipped" with a "batch rejected"
// note, and the on-disk queue is unchanged.
func TestBulkApproval_UnknownID_RollsBackEntireBatch(t *testing.T) {
	srv, dir := srvWithStateDir(t)
	a := enqueuedApproval(t, dir, "merge_pr", &state.SupervisorTarget{PR: 1})

	body := fmt.Sprintf(`{"ids":[%q,"ghost-id"],"verb":"reject"}`, a.ID)
	req := httptest.NewRequest(http.MethodPost, "/api/v1/approvals/bulk", bytes.NewBufferString(body))
	w := httptest.NewRecorder()
	srv.handleApproval(w, req)

	if w.Code != http.StatusBadRequest {
		t.Fatalf("status = %d, want 400 (mixed valid+missing batch must fail atomically); body=%s",
	...
	if resp.OK {
		t.Fatalf("OK=true on a missing-id batch — atomicity contract violated; resp=%+v", resp)
	}
	if resp.Errors == 0 {
		t.Fatalf("Errors=0 despite a missing id in the batch; resp=%+v", resp)
	}
	if resp.Applied != 0 {
		t.Fatalf("Applied=%d on a rejected batch — mutations leaked; resp=%+v", resp.Applied, resp)
	}

	// The disk MUST match the pre-call state — the valid id is still
	// pending, not rejected, because the whole batch was discarded.
	st, _ := state.Load(dir)
	if st.Approvals[0].Status != state.ApprovalStatusPending {
		t.Fatalf("approval %s status = %q on disk; want pending — batch leaked despite missing id",
			st.Approvals[0].ID, st.Approvals[0].Status)
	}
}
