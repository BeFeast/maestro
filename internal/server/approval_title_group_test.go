...
// TestFleetApproval_TargetlessApprovalsNotFolded protects the fix for
// Greptile Comment 3: when both Target and ID are absent, the
// ApprovalGroupKey returns "" instead of "ungrouped" — otherwise
// makeFleetApprovalStates would count unrelated target-less approvals as
// one «regenerated N times» group.
func TestFleetApproval_TargetlessApprovalsNotFolded(t *testing.T) {
	st := &state.State{
		Approvals: []state.Approval{
			{Action: "change_global_config", Status: state.ApprovalStatusPending},
			{Action: "change_global_config", Status: state.ApprovalStatusPending},
		},
	}
	items := makeFleetApprovalStates(fleetProjectState{Name: "scribe"}, st, time.Now().UTC())
	for _, item := range items {
		if item.GroupKey != "" {
			t.Fatalf("target+id-less approval GroupKey = %q, want empty", item.GroupKey)
		}
		if item.GroupSize != 0 {
			t.Fatalf("target+id-less approval GroupSize = %d, want 0 (no folding)", item.GroupSize)
		}
	}
}
