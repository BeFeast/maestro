...
export function workerStatusTaxonomy(worker) {
  ...
  // configured model is unavailable / not accessible (pulled, renamed, or no
  // access), not because the work failed. Distinct from auth failure — the fix
  // is to swap the model id. Retry budget preserved; not a generic red "dead".
...
export function workerNextAction(worker) {
  ...
  if (display === "backend_model_unavailable") {
    const backend = String(worker.provider_limit_backend || worker.backend || "the backend");
    return {
      text: `Backend ${backend}'s model is unavailable or not accessible. Swap the model id to an available one; the retry budget is preserved.`,
      buttons: [{ label: "Open backend health →", action: "openBackendHealth" }],
    };
  }
