/* global React */
// ============================================================
// Maestro MC — Shared atomic components: icons, ECG, sparkline,
// pills, panels, segmented control.
// ============================================================
...
// ============================================================
// ECG / Heartbeat — animated waveform that drives the verdict bar
// ============================================================
function Heartbeat({ tone = "ok", bpm = 18, daemonAlive = true, width = 540, height = 120 }) {
  ...
  const glow = tone === "ok" ? "var(--glow-ok)" : tone === "watch" ? "var(--glow-watch)" : tone === "stuck" ? "var(--glow-stuck)" : "none";
  const path = ecgPath(daemonAlive ? bpm : 0, t, width, height);

  return (
    <div className="ecg" ref={ref}>
      <div className={`ecg-corner ${tone}`}>
        <span className="led" />
        {daemonAlive ? "SUPERVISOR" : "DAEMON OFFLINE"}
      </div>
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
        {/* faint mirrored path for depth */}
...
// Sparkline
// ============================================================
function Sparkline({ data, warm = false }) {
...
// ============================================================
// Pill — strict 4-color contract: ok | watch | stuck | idle | policy
// ============================================================
function Pill({ tone = "idle", children, noDot, title }) {
...
// Panel wrapper
// ============================================================
function Panel({ title, sub, right, children, padded = false }) {
  return (
    <div className="panel">
      {(title || right) && (
...
// ============================================================
// Queue bar (ready/held/blocked) — stacked horizontal
// ============================================================
function QueueBar({ ready = 0, held = 0, blocked = 0, height = 6 }) {
...
Object.assign(window, { Icon, Heartbeat, Sparkline, Stat, Pill, Segmented, Panel, QueueBar });
