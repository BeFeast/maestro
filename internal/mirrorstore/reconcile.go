...
// gh binary.
//
// Every read flows through the package's conditional (ETag / If-None-Match)
...
func (r *Reconciler) Reconcile(ctx context.Context) (ReconcileResult, error) {
	...
	// ---- Issues -------------------------------------------------------------
	ghIssues, err := r.api.ListOpenIssues(nil)
	if err != nil {
...
// case. GitHub was asked for state=open, so absence is unambiguous: the issue is
// no longer open. Returns the number of rows the guarded upsert actually closed.
...
// no longer lists as open. Because IsPRMerged is a mirror-served read (a lagging
// "not merged" only re-polls, but a fabricated "not merged" for a merged PR would
...
// cannot be determined this pass is left for the next one rather than closed with
// a guessed flag.
func (r *Reconciler) closeMissedPRs(ctx context.Context, ghOpen map[int]bool, snap time.Time) (int, error) {
	...
	for _, m := range mirrorOpen {
		...
		if err != nil {
			// Cannot settle merged vs plain-closed right now; skip so the mirror
			// keeps its last known (open) state and the next pass retries, rather
			// than recording a guessed merged flag.
			continue
		}
...
type ReconcileStat struct {
	...
	LastRepairs   int       `json:"last_repairs"` // rows repaired in the last successful pass
