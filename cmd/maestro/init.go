...
func runInitRedirect(w io.Writer) {
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w, "  # 2. Run the exact next[0] command from the approved plan receipt:")
	fmt.Fprintln(w, "  maestro project apply --file <portable-project.yaml> --db <store> --confirm <project-id> --fingerprint <sha256-from-plan> --baseline <baseline-from-plan> --json")
	fmt.Fprintln(w)
