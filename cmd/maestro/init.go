...
func writeInitServiceFile(w io.Writer, binPath, configPath string) error {
	...
	case "linux":
		...
		if err := os.WriteFile(path, []byte(systemdUnit(binPath, configPath, currentPATH)), 0644); err != nil {
			return err
		}
		fmt.Fprintf(w, "\u2705 Created %s\n", path)
	case "darwin":
		...
		if err := os.WriteFile(path, []byte(launchdPlist(binPath, configPath, currentPATH)), 0644); err != nil {
			return err
		}
		fmt.Fprintf(w, "\u2705 Created %s\n", path)
	}
