...
The snapshot reads (`ListOpenIssues`, `ListOpenPRs`) flow through the gh wrapper's
conditional (ETag / `If-None-Match`) layer (#797). A repo whose lists have not
changed since the last pass answers **304 Not Modified** and consumes **near-zero
core quota**; the reconciler then writes nothing. The per-PR `IsPRMerged` call is
made only for a PR that dropped out of the open set (a rare per-drift call, not a
per-cycle one). The loop also skips a pass while the shared primary rate-limit
gate is armed (#812), since the reads would only fail against an empty budget.
